...
MKillBuff()			/* delete a buffer */
{
	...
	LowCase(namearg);
	if ((cnt=CFindBuff(namearg))<0) {
		Error("Does not exist");
		return;
		}
	if (cnt==cbuff) MSwitchTo();
	if (cnt==cbuff) {
		Error("Can't delete current");
		return;
		}
	if (BModp(buffs[cnt].bbuff) && !Ask("Delete Modified Buffer?")) return;
	if (divide<TMaxRow()-2 && cnt==altbuff) MOneWind();
	BDelBuff(buffs[cnt].bbuff);
	buffs[cnt].bbuff=NULL;
