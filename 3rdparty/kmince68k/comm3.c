...
/*
 * three funs: MStrtRem() - start remembering (bound to ^X-( ),
 *	       MStpRem()  - stop remembering (bound to ^X-) ), and
 *	       MExcKBMc() - execute keyboard macro (bound to ^X-e).
 */
 
MStrtRem()
{
    argp = FALSE; arg = 0;
    if (KBexecuting) {Error("Recursion illegal!"); abort = TRUE; return;}
    if (remember) {Error("Already remembering!"); return;} 
    remember = TRUE;
    QInit(&kbmacro,MAXKBMACL);
    Echo("Start remembering...");
    }
MStpRem()
{
    argp = FALSE; arg = 0;
    if (KBexecuting) {abort = TRUE; return;}
    if (!remember) {Error("Not remembering!"); return;}
    remember = FALSE;
    Echo("Keyboard macro defined...");
    }
MExcKBMc()
{
    int Kabort, Karg, Kargp, Kcmnd, (*Klfunct)();
    ...
    lfunct = Klfunct;
    KBexecuting = FALSE;
    if (GAbort) {argp = FALSE; arg = 0;}
    }

/*************** end of keyboard macro hack *****************/
#endif
#ifdef STRIDE
/*************** Stride insert time hack ****************/

/*
 * one function MInsTime() - inserts the current time into the buffer
 */
...
MInsTime()
{
    ...
    register char *p;
    static REALTIME tbuff;

    gettime(&tbuff);
    sprntime(timestr,&tbuff);
    for (p=timestr;*p != '\0';p++) BInsert(*p);
    }
#endif
