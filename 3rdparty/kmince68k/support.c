...
#ifdef LISPMODE
	if (!strcmp(tmname,"lisp")) return('l');
#endif
#endif
	...
	char *mesg;
{
	...
	TSetPoint(trow,tcol);
	TForce();
	if (KBexecuting) {abort = TRUE; GAbort = TRUE;}
	}
...
int IsClose()				/* do we have a closing character */
{
	return(Buff()==')' || Buff()==']' || Buff()=='}' ||
		Buff()=='"' || Buff()=='\'');
	}

int IsGray()				/* tell if current char is white space */
{
	...
	int (*from)(), (*to)();
{
	...
	int icnt;
#endif
	for (icnt=383; icnt>=0; --icnt) {
		if (functs[icnt]==from) functs[icnt]=to;
		TKbChk();
		}
	}
